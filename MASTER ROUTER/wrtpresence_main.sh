...
plCheckDevicesByCounters() {
	...
	while (true); do
		...
		if (cat "${ASSOCIATIONS_DTO}" 2> /dev/null | grep -Fi "=${DEVICE_MAC[${DPTE_DEVICE_I}]}=" | grep -vq "=${DEVICE_DISCONNECTED_IDX}$"); then
			# 
			# Device PRESENT.
			echo "${DEVICE_MAC[${DPTE_DEVICE_I}]}" >> "${PRESENT_DEVICES_DTO}.new"
			# 
			# Did the device state change from AWAY to PRESENT?
			if ( ! grep -iq "${DEVICE_MAC[${DPTE_DEVICE_I}]}$" "${PRESENT_DEVICES_DTO}" ); then
				#
				# DEVICE_ACTION_PRESENT.
				#
				fifoOut "DEV/${DEVICE_MAC[${DPTE_DEVICE_I}]}=present"
				#
			fi
		else
			# 
			...
			if ( grep -iq "${DEVICE_MAC[${DPTE_DEVICE_I}]}$" "${PRESENT_DEVICES_DTO}" ); then
				...
				# DEVICE_ACTION_AWAY.
				# 
				fifoOut "DEV/${DEVICE_MAC[${DPTE_DEVICE_I}]}=away"
				#
			fi
		fi
		#
...
plAddClient() {
	...
	sleep 1
	"${MOSQUITTO_PUB}" -r -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t wrtpresence/"${TMP_PLAC_MAC_ADDR//:/}"/state -u "${MQTT_USER}" -P "${MQTT_PASSWORD}" -m "home"
	"${MOSQUITTO_PUB}" -r -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t wrtpresence/"${TMP_PLAC_MAC_ADDR//:/}"/attributes -u "${MQTT_USER}" -P "${MQTT_PASSWORD}" -m "$attributes"
	#
	return
}
...
plMarkClientAsDisconnected() {
	...
	# Publish to MQTT
	ishome=$(awk -F "["="]" '$2=="'${TMP_PLAC_MAC_ADDR}'" && $3==0 && !found { print "home"; found++ } END { if (!found) print "not_home" }' "${ASSOCIATIONS_DTO}")
	"${MOSQUITTO_PUB}" -r -h "${MQTT_HOST}" -p "${MQTT_PORT}" -t wrtpresence/"${TMP_PLAC_MAC_ADDR//:/}"/state -u "${MQTT_USER}" -P "${MQTT_PASSWORD}" -m "$ishome"
	#
	return
}
